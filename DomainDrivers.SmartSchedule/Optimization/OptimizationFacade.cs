...
public class OptimizationFacade
{
        ...
        IList<Item> items, TotalCapacity totalCapacity)
    {
		...
		// Krzyżujemy poprzez branie w pierwszej kolejności projektów, które są zawarte w obu kandydatach.
		// Następnie w jednym z nich, a na koniec dodajemy projekty, których nie ma w żadnym kandydacie.
    ...
    {
        // Aby obsłużyć wiele umiejętności per zasób, powinno być ResourceId
        public ICapacityDimension CapacityDimension { get; set; } = capacityDimension;

		// Aby obsłużyć wiele umiejętności per zasób, powinniśmy używać kalendarzy dostępności zamiast trzymać użyte TimeSloty
